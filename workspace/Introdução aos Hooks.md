...
**Você pode começar a aprender Hooks [na próxima página](https://pt-br.reactjs.org/docs/hooks-overview.html).** Nesta página, nós iremos continuar explicando porque nós estamos adicionando Hooks em React e como eles podem ajudar a escrever boas aplicações.

> Nota
>
> React 16.8.0 é o primeiro release com suporte a Hooks. Ao atualizar, não se esqueça de atualizar todos os pacotes, incluindo React DOM. React Native suporta Hooks desde [a versão 0.59 de React Native](https://reactnative.dev/blog/2019/03/12/releasing-react-native-059).
...
<iframe src="https://www.youtube.com/embed/dpw9EHDh2bM" frameborder="0" allowfullscreen="" style="margin: 0px; padding: 0px; box-sizing: inherit; position: absolute; top: 0px; left: 0px; width: 794.094px; height: 447.125px;"></iframe>
...
**Não temos planos de remover classes do React.** Você pode aprender mais sobre a estratégia de adoção gradual para Hooks na [seção inferior](https://pt-br.reactjs.org/docs/hooks-intro.html#gradual-adoption-strategy) desta página.
...
**Se você só quiser começar a aprender Hooks, sinta-se livre para [pular direto para a próxima página!](https://pt-br.reactjs.org/docs/hooks-overview.html)** Você também pode continuar lendo esta página para aprender mais sobre o porquê de estarmos adicionando Hooks e como nós iremos começar a usá-los sem reescrever nossas aplicações.
...
React não oferece uma forma de “vincular” comportamento reutilizável em um componente (por exemplo, conectar um componente a uma store). Se você já trabalhou com React por um tempo, você pode estar familiarizado com padrões como [render props](https://pt-br.reactjs.org/docs/render-props.html) e [higher-order components](https://pt-br.reactjs.org/docs/higher-order-components.html) que tentam resolver isso. Mas estes padrões requerem que você reestruture seus componentes quando você os usa, o que pode parecer atrapalhado e tornar o código mais difícil de entender. Se você ver uma aplicação React típica com React DevTools, você provavelmente irá encontrar um “inferno de wrappers” de componentes rodeados de camadas de providers, consumers, high-order components, render props, e outras abstrações. Apesar de podermos [filtrar eles na DevTools](https://github.com/facebook/react-devtools/pull/503), isso aponta para um problema mais profundo: React precisa de uma primitiva melhor para compartilhar lógica com estado.
...
Nós iremos discutir mais sobre isso em [Construindo Seus Próprios Hooks](https://pt-br.reactjs.org/docs/hooks-custom.html).
...
Nós iremos discutir mais sobre isso em [Usando o Effect Hook](https://pt-br.reactjs.org/docs/hooks-effect.html#tip-use-multiple-effects-to-separate-concerns).
...
> Exemplos
>
> [Hooks at a Glance](https://pt-br.reactjs.org/docs/hooks-overview.html) é um bom lugar para começar a aprender Hooks.
...
Nós preparamos uma [página de Perguntas Frequentes sobre Hooks](https://pt-br.reactjs.org/docs/hooks-faq.html) que respondem às perguntas mais comuns sobre Hooks.

## Próximos Passos

Ao final desta página, você deverá ter uma ideia aproximada de quais problemas *Hooks* estão resolvendo, mas muitos detalhes provavelmente não estão claros. Não se preocupe! **Vamos agora para a [próxima página](https://pt-br.reactjs.org/docs/hooks-overview.html) onde nós iremos aprender sobre \*Hooks\* com exemplos.**

Esta página é útil?[Edite esta página](https://github.com/reactjs/reactjs.org/tree/main/content/docs/hooks-intro.md)